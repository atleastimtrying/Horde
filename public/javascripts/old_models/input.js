(function() {
  ...
  window.Input = (function() {
    ...
    Input.prototype.keyPressDown = function(event) {
      this.keyDown = true;
      if (event.keyCode === 87) {
        this.app.localPlayer.instructions = [0, -1];
      }
      if (event.keyCode === 83) {
        this.app.localPlayer.instructions = [0, 1];
      }
      if (event.keyCode === 65) {
        this.app.localPlayer.instructions = [-1, 0];
      }
      if (event.keyCode === 68) {
        this.app.localPlayer.instructions = [1, 0];
      }
      if (event.keyCode === 68) {
        return this.app.localPlayer.instructions = [1, 0];
      }
    };
    ...
    Input.prototype.clicked = function(event) {
      if (event.which === 1) {
        this.app.localPlayer.shoot();
      }
      if (event.which === 3) {
        return this.app.localPlayer.melee();
      }
    };
    return Input;
  })();
